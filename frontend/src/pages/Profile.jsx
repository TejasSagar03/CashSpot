...
export default function Profile({ user, openSettings }) {
  ...
  const handleAvatarChange = async (event) => {
    ...
    if (file) {
      try {
        ...
        showAlert("ERROR", "Failed to process avatar file.");
      }
    }
  };
  ...
  const handleBannerChange = async (event) => {
    ...
    if (file) {
      try {
        ...
        showAlert("ERROR", "Failed to process banner file.");
      }
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (bannerPreview && bannerPreview !== localStorage.getItem('cashspot_user_banner')) {
          localStorage.setItem('cashspot_user_banner', bannerPreview);
          const userRef = doc(db, "users", user.uid);
          await setDoc(userRef, { bannerURL: bannerPreview }, { merge: true });
      }

      showAlert("SUCCESS", "Profile parameters updated.");
      setIsEditing(false);
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      showAlert("SUCCESS", "Authentication key updated successfully.");
            ...
            >
              ...
              {isEditing && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center p-4">
                  <button onClick={() => document.getElementById('banner-upload').click()} className="flex gap-2 items-center px-4 py-2 bg-black/60 text-white rounded-full text-xs font-bold hover:bg-black/80 transition-colors">
            ...
            <div className="px-6 md:px-10 pb-8 relative">
              <div className="flex justify-between items-end -mt-12 md:-mt-16 mb-6">
                ...
                <div className="relative">
                  ...
                  {isEditing && (
                    <div className="absolute bottom-1 right-1 md:bottom-2 md:right-2 w-7 h-7 bg-black text-white rounded-full border-[3px] border-white dark:border-[#0a0a0a] z-20 flex items-center justify-center shadow-md cursor-pointer hover:bg-gray-800" onClick={() => document.getElementById('avatar-upload').click()} title="Change Avatar">
                ...
                <div className="flex gap-2 md:gap-3">
                  {isEditing ? (
                    ...
                    <>
                      <button type="button" onClick={() => setShowManual(true)} className="w-10 h-10 flex items-center justify-center border border-gray-200 dark:border-gray-800 rounded-full text-black dark:text-white hover:bg-gray-50 dark:hover:bg-[#111] transition-colors bg-white dark:bg-[#0a0a0a] shadow-sm cursor-pointer" title="Intel Manual">
